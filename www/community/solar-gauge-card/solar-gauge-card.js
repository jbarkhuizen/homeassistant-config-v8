 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */}class pt{constructor(t,e){this.solarColor="#ffee3A",this.gridOutColor="#488fc2",this.gridInColor="#8353d1",this.homeColor="#ffa500",this.batteryPowerOutColor="#0bac3b",this.batteryPowerInColor="#83d3d1",this.batteryColor="green",this.gridUpdateMap=new Map([[pt.IN,{valueColor:"green",text:"Grid In",active:!0}],[pt.ZERO,{valueColor:"",text:"Grid",active:!1}],[pt.OUT,{valueColor:"red",text:"Grid Out",active:!0}]]),this.batteryUpdateMap=new Map([[pt.IN,{valueColor:"green",text:"Battery In",active:!0}],[pt.ZERO,{valueColor:"",text:"Battery",active:!1}],[pt.OUT,{valueColor:"red",text:"Battery Out",active:!0}]]),this.hass=t,this.config=e}initializeEntity(t){let e=0,i=!0;if(t.entity)e=Number(t.entity.state),t.color=e>0?t.out.color:t.in.color,i=!0;else if(t.out.entity&&t.in.entity){const i=Number(t.out.entity.state),s=Number(t.in.entity.state);if(i>0)t.entity=t.out.entity,t.color=t.out.color,e=i;else{if(!(s>0))throw new Error("At least one of the 'out' or 'in' entities must be positive.");t.entity=t.in.entity,t.color=t.in.color,e=-1*s}}else{if(t.out.entity&&!t.in.entity||!t.out.entity&&t.in.entity)throw new Error("Invalid configuration.");i=!1}return{entity:t.entity,value:e,textColor:t.color,active:i}}customizeInOutEntity(t,e){let i=this.initializeEntity(t);return i=i.value<0?{...i,...e.get(pt.IN)}:0==i.value?{...i,...e.get(pt.ZERO)}:{...i,...e.get(pt.OUT)},i}customizeSimpleEntity(t){return{entity:t.entity,value:Number(t.entity.state),textColor:t.color,text:t.text}}buildGridPowerEntity(t,e){return{entity:t.states[e.gridPower.entity],color:"",out:{entity:t.states[e.gridPower?.out?.entity],color:this.gridOutColor},in:{entity:t.states[e.gridPower?.in?.entity],color:this.gridInColor}}}buildBatteryPowerEntity(t,e){return{entity:t.states[e.batteryPower?.entity],color:"",out:{entity:t.states[e.batteryPower?.out?.entity],color:this.batteryPowerOutColor},in:{entity:t.states[e.batteryPower?.in?.entity],color:this.batteryPowerInColor}}}buildSolarPowerEntity(t,e){return{entity:t.states[e.solarPower.entity],color:this.solarColor,text:"Solar"}}buildHomeConsumptionEntity(t,e){return{entity:t.states[e.homeConsumption.entity],color:this.homeColor,text:"Home"}}buildBatteryEntity(t,e){return{capacity:{entity:t.states[e.battery?.capacity?.entity],color:this.batteryColor,precision:e.battery?.capacity?.precision},temperature:{entity:t.states[e.battery?.temperature?.entity]}}}}var yt,gt;pt.IN="in",pt.ZERO="zero",pt.OUT="out";let $t=class extends ht{constructor(){super(...arguments),yt.set(this,void 0),gt.set(this,void 0),this.svgWidth="300px",this.svgHeight="200px",this.svgX=150,this.svgY=60,this.minStrokeWidth=5,this.maxStrokeWidth=20,this.calculatedStrokeWidth=20}get _hass(){return e(this,yt,"f")}set _hass(t){i(this,yt,t,"f")}get _config(){return e(this,gt,"f")}set _config(t){i(this,gt,t,"f")}_gauge(t,e,i,s){const r=2*Math.PI*t,o=Math.min(Math.max(e/i*r/2,0),r/2),n=Math.max(r-o,0);return X`
          <circle id="gauge-${s}"
            cx="${this.svgX}"
            cy="${this.svgY}"
            r="${t}"
          ...
          </circle>`}getFinalStrokeWidth(t,e){const i=e||Math.trunc(t/200);return Math.max(Math.min(i,this.maxStrokeWidth),this.minStrokeWidth)}isPositiveInteger(t){return!isNaN(t)&&Number.isInteger(t)&&t>=0}drawBattery(t){if(t.capacity.entity){let e=Number(t.capacity.entity.state);if(isNaN(e)&&(e=0),void 0!==t.capacity.precision){let i=Number(t.capacity.precision);if(this.isPositiveInteger(i)){const t=Math.pow(10,i);e=Math.round(e*t)/t}else console.log("Battery capacity precision value "+t.capacity.precision+" is invalid")}const i=130,s=6,r=2,o=5*s+4*r,n=12,a=i+o,h=160,l=0,c=i+(e-l)*(a-i)/(100-l),d=t.capacity.color;return X`
         ...
         ${t.temperature.entity?X`
          <text id="battery-temperature-value" x="${i}" y="${h-16}" class="svg-text svg-text-middle" style="font-size: 10px;">${Number(t.temperature.entity.state)}${t.temperature.entity.attributes.unit_of_measurement}</text>
           ...
           <rect id="battery-top" x="${i+o+2}" y="${h-5}" rx="3" ry="3" width="5" height="10" style="fill:${d}"></rect>
        `}}setConfig(t){if(!(t.solarPower.entity&&t.homeConsumption.entity&&(t.gridPower.entity||t.gridPower.out?.entity&&t.gridPower.in?.entity)))throw new Error("You need to define mandatory entities: 'solarPower', 'homeConsumption' and 'gridPower' in the card configuration.");console.log("All set. Enjoy!!!"),this._config=t}set hass(t){this._hass=t,this.requestUpdate()}render(){if(this._hass){const t=new pt(this._hass,this._config),e=t.buildGridPowerEntity(this._hass,this._config);this.gridEntity=t.customizeInOutEntity(e,t.gridUpdateMap);const i=t.buildBatteryPowerEntity(this._hass,this._config);this.batteryPowerEntity=t.customizeInOutEntity(i,t.batteryUpdateMap);const s=t.buildSolarPowerEntity(this._hass,this._config);this.solarEntity=t.customizeSimpleEntity(s);const r=t.buildHomeConsumptionEntity(this._hass,this._config);this.homeConsumptionEntity=t.customizeSimpleEntity(r);const o=t.buildBatteryEntity(this._hass,this._config),n=this.drawBattery(o),a=this._config.gaugeWidth;this.style.setProperty("--solar-color",s.color),this.style.setProperty("--grid-out-color",e.out.color),this.style.setProperty("--grid-in-color",e.in.color),this.style.setProperty("--battery-out-color",i.out.color),this.style.setProperty("--battery-in-color",i.in.color),this.style.setProperty("--home-color",r.color);const h=Math.max(0,this.gridEntity.value),l=Math.abs(Math.min(0,this.gridEntity.value)),c=Math.max(0,this.batteryPowerEntity.value),d=Math.abs(Math.min(0,this.batteryPowerEntity.value)),u=this.solarEntity.value+c+h,p=this.homeConsumptionEntity.value+d+l,y=100;this.calculatedStrokeWidth=this.getFinalStrokeWidth(u,a);const g=y-this.calculatedStrokeWidth;return Y` <ha-card>
      <svg width="${this.svgWidth}" height="${this.svgHeight}">
       ${this._gauge(y,u,u,"circle-grid-out")}
       ${this._gauge(y,this.solarEntity.value+c,u,"circle-battery-out")}
       ${this._gauge(y,this.solarEntity.value,u,"circle-solar")}
       ${this._gauge(g,p,p,"circle-grid-in")}
       ${this._gauge(g,this.homeConsumptionEntity.value+d,p,"circle-battery-in")}
       ${this._gauge(g,this.homeConsumptionEntity.value,p,"circle-home")}

    <text id="gauge-home-text" x="${this.svgX-y-this.calculatedStrokeWidth/2}" y="${2.5*this.svgY}" class="svg-text svg-text-description svg-text-left" style="fill:${this.homeConsumptionEntity.textColor}">
        ${this.homeConsumptionEntity.text}
    </text>
    <text id="gauge-home-value" x="${this.svgX-y-this.calculatedStrokeWidth/2}" y="${2.7*this.svgY}" class="svg-text svg-text-value svg-text-left">
    ...
    <text id="gauge-solar-text" x="${this.svgX+y+this.calculatedStrokeWidth/2}" y="${2.5*this.svgY}" class="svg-text svg-text-description svg-text-right" style="fill: ${this.solarEntity.textColor}">
        ${this.solarEntity.text}
    </text>
    <text id="gauge-solar-value" x="${this.svgX+y+this.calculatedStrokeWidth/2}" y="${2.7*this.svgY}" class="svg-text svg-text-value svg-text-right" style="fill: ${this.solarEntity.valueColor}">
        ${this.solarEntity.value} ${this.solarEntity.entity.attributes.unit_of_measurement}
    </text>
    <text id="gauge-battery-text" x="${this.svgX+y+this.calculatedStrokeWidth/2}" y="${2.9*this.svgY}" class="svg-text svg-text-description svg-text-right" style="fill: ${this.batteryPowerEntity.textColor||"none"}">
        ${this.batteryPowerEntity.text||""}
    </text>
    <text id="gauge-battery-value" x="${this.svgX+y+this.calculatedStrokeWidth/2}" y="${3.1*this.svgY}" class="svg-text svg-text-value svg-text-right" style="fill: ${this.batteryPowerEntity.valueColor||"none"}">
      ...
      `}return Y``}};yt=new WeakMap,gt=new WeakMap,$t.styles=((t,...e)=>{const i=1===t.length?t[0]:e.reduce(((e,i,s)=>e+(t=>{if(!0===t._$cssResult$)return t.cssText;if("number"==typeof t)return t;throw Error("Value passed to 'css' function must be a 'css' function result: "+t+". Use 'unsafeCSS' to pass non-literal values, but take care to ensure page security.")})(i)+t[s+1]),t[0]);return new a(i,t,o)})`
